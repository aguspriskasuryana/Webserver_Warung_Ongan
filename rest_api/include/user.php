...
class User{
	...
	public function loginUsers($email, $password, $token){
			
		$json = array();
		
		$queryxx = "delete from token WHERE TOKEN_ID = '$token' " ;	
		$resultxx = mysqli_query($this->db->getDb(), $queryxx);
		...
		if(mysqli_num_rows($resultX) > 0){
			...
			$us_id = $row[$this->FLD_USER_ID];
			//$json[$this->FLD_EMAIL] = $row[$this->FLD_EMAIL];
			}
			
			$querydelete = "delete from token WHERE TOKEN = '$us_id' " ;	
			$json['querydelete']= $querydelete;
			$resultX = mysqli_query($this->db->getDb(), $querydelete);
			
			$querya = "INSERT INTO token
				 (TOKEN_ID ,TOKEN) VALUES 
			('$token','$us_id');";
			$inserted = mysqli_query($this->db->getDb(), $querya);
			$json['querya'] = $querya;
