...
switch ($command) {
    ...
    case $commandObject->CMD_SAVE:
			...
			$json_registration = $userObject->createUser($username, $password, $fullname, $email,"","", "", "",$birthdate,"1","","0");
    ...
    case $commandObject->CMD_UPDATE:
        ...
        if(!empty($userid) && !empty($username) && !empty($password) && !empty($email)){
			$json = $userObject->updateUser($userid,$username, $password, $fullname, $email,$address,$fileattach, $phonenumber, $birthplace,$birthdate,$sex,$joindate,$userprivilage);
			echo json_encode($json);
		}
        break;
	case $commandObject->CMD_DELETE:
		if(!empty($userid) && $userid != 0 ){
			$json = $userObject->deleteUser($userid);
